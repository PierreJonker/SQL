...
FROM 'C:\Bootcamp\Java_Bootcamp\SQL\My_Work\Chapter14\farmers_markets.csv'
...
SELECT longitude, latitude, geog_point, ST_AsText(geog_point)
...
SELECT market_name, city,
       round((ST_Distance(geog_point, ST_GeogFromText('POINT(-93.6204386 41.5853202)') ) / 1609.344)::numeric(8,5), 2) AS miles_from_dt
FROM farmers_markets
WHERE ST_DWithin(geog_point, ST_GeogFromText('POINT(-93.6204386 41.5853202)'), 10000)
...
SELECT water.fullname AS waterway, roads.rttyp, roads.fullname AS road, ST_AsText(ST_Intersection(water.geom, roads.geom))
FROM santafe_linearwater_2016 water JOIN santafe_roads_2016 roads
    ON ST_Intersects(water.geom, roads.geom)
WHERE water.fullname = 'Santa Fe Riv'
ORDER BY roads.fullname
...
-- 1. Finding the area of each state in square miles and identifying states larger than Yukon-Koyukuk.
SELECT statefp10 AS st, round((sum(ST_Area(geom::geography) / 2589988.110336))::numeric, 2) AS square_miles
FROM us_counties_2010_shp
GROUP BY statefp10
ORDER BY square_miles DESC;

-- 2. Using ST_Distance() to determine miles between two farmers' markets.
WITH
...
-- 3. Performing a spatial join to find missing county names based on the location of each market.
SELECT census.name10, census.statefp10, markets.market_name, markets.county, markets.st
